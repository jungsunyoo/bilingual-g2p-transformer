...
#                      if not line.startswith("<")]
    _prepro = lambda x:  [line.strip() for line in open(x, 'rt', encoding='UTF-8').read().split("\n") \
                      if not line.startswith("<")]
    ...
    _prepro = lambda x: [re.sub("<[^>]+>", "", line).strip() \
                     for line in open(x, 'rt', encoding='UTF-8').read().split("\n") \
...
#        with open(fname, "w") as fout:
    ...
    print("train1:", open("iwslt2016/segmented/train.de.bpe",'rt', encoding='UTF-8').readline())
    print("train2:", open("iwslt2016/segmented/train.en.bpe",'rt', encoding='UTF-8').readline())
    print("eval1:", open("iwslt2016/segmented/eval.de.bpe",'rt', encoding='UTF-8').readline())
    print("eval2:", open("iwslt2016/segmented/eval.en.bpe",'rt', encoding='UTF-8').readline())
    print("test1:", open("iwslt2016/segmented/test.de.bpe",'rt', encoding='UTF-8').readline())
